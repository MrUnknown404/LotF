...
public final class Console {
	...
	public static void print(String string) {
		if (DISABLED_TYPES.length != 0) {
			for (int i = 0; i < DISABLED_TYPES.length; i++) {
				if (DISABLED_TYPES[i] == WarningType.Debug) {
					return;
				}
			}
		}
		
		System.out.println("[" + new SimpleDateFormat("hh:mm:ss:SSS").format(new Date()) + "] [" + WarningType.Debug +  "] [" + getCallerInfo(Console.class.getName()) + "] : " + string);
	}
